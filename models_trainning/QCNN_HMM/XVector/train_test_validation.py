...
class SpeakerIdentification:
    def __init__(self, n_qubits=7, n_hmm_components=1, use_gpu=True):
        ...
        self.speakers = {}
        self.hmm_models = {}
        self.save_dir = "xvector_qcnn_hmm_single"
        ...
        try:
            ...
            print("Successfully loaded SpeechBrain X-vector model")
    ...
    def predict(self, test_audio_path, start_time, end_time):
        xvector = self.extract_xvector(test_audio_path, start_time, end_time)
...
def cross_validate(base_folder, k=3, save_dir="crossval_models", log_file="crossval_log.txt", use_gpu=True):
    ...
    append_log([f"🔁 Cross-Validation Start - {k} folds with X-vector features\n"])
