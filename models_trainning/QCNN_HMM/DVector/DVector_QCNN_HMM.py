...
class SpeakerIdentification:
    def __init__(self, n_qubits=4, n_hmm_components=2):
        ...
        # Load pretrained d-vector model (ECAPA-TDNN)
    ...
    def process_qcnn(self, dvector):
        # Process d-vector with quantum circuit
        if len(dvector) < self.n_qubits:
            dvector = np.pad(dvector, (0, self.n_qubits - len(dvector)))
        else:
            dvector = dvector[:self.n_qubits]
        
        q_features = self.qcnn(dvector, self.weights)
    ...
    def predict(self, test_audio_path, start_time, end_time):
        test_dvector = self.extract_dvector(test_audio_path, start_time, end_time)
        test_features = self.process_qcnn(test_dvector)
        ...
        # Điều chỉnh số frame cho đúng với HMM
