...
def cross_validate_rnn(features, labels, speakers, num_folds=3):
    ...
    for train_idx, test_idx in skf.split(features, encoded_labels):
        ...
        y_train_cat = to_categorical(y_train, num_classes=len(speakers))
        y_test_cat = to_categorical(y_test, num_classes=len(speakers))
        ...
        model = Sequential([
            ...
            Dropout(0.2),
            Dense(32, activation='relu'),
            Dense(len(speakers), activation='softmax')
        ])
...
if __name__ == "__main__":
    ...
    cross_validate_rnn(features, labels, speakers)
