...
def train_speaker_recognition_system():
    ...
    xvectors = extract_xvectors(all_segments, xvector_model)
    ...
    speaker_xvectors = defaultdict(list)
    speaker_encoded_labels = defaultdict(list)
    speaker_indices = defaultdict(list)
    
    for i, (xvector, label) in enumerate(zip(xvectors, all_labels)):
        speaker_xvectors[label].append(xvector)
    ...
    xvectors_tensor = torch.tensor(xvectors, dtype=torch.float32)
    labels_tensor = torch.tensor(encoded_labels, dtype=torch.long)
    with open('XVector_QCNN.txt', 'w') as f:
        f.write("XVector QCNN Speaker Recognition Results\n")
        f.write("======================================\n\n")
    for fold in range(N_FOLDS):
        ...
        X_train = xvectors_tensor[train_indices]
        y_train = labels_tensor[train_indices]
        X_test = xvectors_tensor[test_indices]
        ...
        torch.save(qcnn_model.state_dict(), f'qcnn_speaker_recognition_fold{fold+1}.pth')
        with open('XVector_QCNN.txt', 'a') as f:
    ...
    print("\nTraining complete! Results saved to XVector_QCNN.txt")
