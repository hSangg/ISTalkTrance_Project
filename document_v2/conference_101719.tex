...
\begin{figure}[H]
    ...
    \resizebox{0.45\textwidth}{!}{%
        \begin{tikzpicture}[
            ...
            % Nodes
            \node[box] (adc) {A/D\\conversion};
            \node[box, right=of adc] (pre) {Preemphasis};
            \node[box, right=of pre] (win) {Windowing};
            \node[box, right=of win] (dft) {FFT};
            \node[box, below=of dft] (mel) {Mel\\filterbank};
            \node[box, below=of mel] (log) {log()};
            \node[box, below=of log] (idft) {IDFT};
            \node[box, below=of idft] (dyn) {Dynamic\\features};
            \node[box, left=of dyn] (feat) {Feature\\Transform};

            % Energy line
            \node[coordinate, left=1cm of mel] (energyanchor) {};
            \draw[line] (win) |- node[near end, left] {energy} (energyanchor) |- (dyn);
...
\subsection{Hidden Markov Models - Wavelet}
